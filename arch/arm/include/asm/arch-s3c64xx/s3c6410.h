...
#define APLL_VAL	((1 << 31) | (STARTUP_AMDIV << 16) | \
	(STARTUP_APDIV << 8) | STARTUP_ASDIV)
#define STARTUP_APLL	(((CONFIG_SYS_CLK_FREQ >> STARTUP_ASDIV) / \
	STARTUP_APDIV) * STARTUP_AMDIV)
#define STARTUP_APLLDIV		0   /* ARMCLK = DOUTAPLL / (ARM_RATIO + 1) */

/* MPLL=533MHz */
#define STARTUP_MMDIV		533
#define STARTUP_MPDIV		6
#define STARTUP_MSDIV		1

#define MPLL_VAL	((1 << 31) | (STARTUP_MMDIV << 16) | \
	(STARTUP_MPDIV << 8) | STARTUP_MSDIV)
#define STARTUP_MPLL	(((CONFIG_SYS_CLK_FREQ >> STARTUP_MSDIV) / \
	STARTUP_MPDIV) * STARTUP_MMDIV)

/*
 * HCLKX2IN=533MHz
 * HCLKX2=HCLKX2IN/(HCLKX2_RATIO+1)=266MHz
 * HCLK=HCLKX2/(HCLK_RATIO+1)=133MHz
 * PCLK=HCLKX2/(PCLK_RATIO+1)=66MHz
 * DOUTMPLL =MOUTMPLL/(MPLL_RATIO+1)=266MHz
 */
#define STARTUP_HCLKX2DIV	1   /* HCLKX2 = HCLKX2IN / (HCLKX2_RATIO + 1) */
#define STARTUP_HCLKDIV		1   /* HCLK = HCLKX2 / (HCLK_RATIO + 1) */
#define	STARTUP_PCLKDIV		3   /* PCLK = HCLKX2 / (PCLK_RATIO + 1) */
#define STARTUP_MPLLDIV		1   /* DOUTMPLL = MOUTMPLL / (MPLL_RATIO + 1) */
